...
const string strMessageMagic = "Maxi Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if ((nBestHeight > 1) && (nBestHeight < 10800))  // 100 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 1;
    }
    
    else if (nBestHeight < 25200) // 600 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 6;
    }

    else if (nBestHeight < 46800) // 1200 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 12;
    }

    else if (nBestHeight < 90000) // 1500 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 15;
    }

    else if (nBestHeight < 133200) // 1000 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 10;
    }

    else if (nBestHeight < 198000) // 800 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 8;
    }

    else if (nBestHeight < 234000) // 500 %
    {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Maxi", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The crest and crowning of all good, Life's final star, is brotherhood.";
        CTransaction txNew;
        txNew.nTime = 1525280262;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1525280262;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 13588 : 13588;
        ...
        assert(block.hashMerkleRoot == uint256("0xea2bacf8af43ecc9bb4a51126c62683e910fb7ca07dc7b2aa1cae1f0285f5e63"));
...
unsigned char pchMessageStart[4] = { 0x85, 0xf0, 0xd8, 0xe9 };
